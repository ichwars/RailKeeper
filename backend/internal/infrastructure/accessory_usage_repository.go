...
func (r *AccessoryRepository) GetUsageHistory(
	...
	for rows.Next() {
		...
		if err := rows.Scan(&event.ID, &event.Type, &event.ProductID, &event.ReservationID,
			...
			&event.VehicleID, &event.LayoutID, &event.LayoutUnitID, &event.TechnicalPositionID, &event.Placement,
...
SELECT * FROM (
  SELECT
    ...
    COALESCE(layout_unit_id, ''),
    COALESCE((SELECT position_id FROM accessory_reservation_positions WHERE reservation_id=id), ''),
    placement, digital_address, decoder_output, connection,
  ...
  SELECT
    ...
    COALESCE(layout_unit_id, ''),
    COALESCE((SELECT position_id FROM accessory_installation_positions WHERE installation_id=id), ''),
    placement, digital_address, decoder_output, connection,
  ...
  SELECT
    ...
    COALESCE(installation.layout_unit_id, ''),
    COALESCE((SELECT position_id FROM accessory_installation_positions
              WHERE installation_id=installation.id), ''),
    installation.placement, installation.digital_address,
