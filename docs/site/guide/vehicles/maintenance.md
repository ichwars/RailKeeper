...
Stable v0.1.20 can display attachments that already contain a maintenance reference, but the
...
4. If a linked attachment must retain the association, keep the maintenance entry. Stable
   v0.1.20 has no attachment-link editor. Remove an attachment only after backup and content
