...
The language choice is stored only in the current browser in stable v0.1.20.1. It does not follow the
...
- **Layout** is selectable in v0.1.20.1 but remains an unpublished development workspace and is not
...
RailKeeper stores and synchronizes both choices. Stable v0.1.20.1 does not yet connect these settings
...
Stable v0.1.20.1 stores this value but does not route all print actions to the chosen destination.
...
The list can include **Layout** even though stable v0.1.20.1 does not publish that workspace in the
...
| The interface language differs on another device | Set it again there. Language is browser-local in v0.1.20.1. |
...
| The selected printer or date format has no effect | These preferences are stored but are not yet used globally by every v0.1.20.1 workflow. |
