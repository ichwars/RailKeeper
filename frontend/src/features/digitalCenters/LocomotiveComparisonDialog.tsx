...
export function LocomotiveComparisonDialog({ item, onClose }: {
  ...
  return (
    <div className="digital-comparison-layer" role="dialog" aria-modal="true"
      aria-label={`Lok-Vergleich ${item.name}`}>
      <section className="digital-comparison-dialog">
        <header><div><p className="eyebrow">LOK-ABGLEICH</p><h2>{item.name}</h2></div>
          <button type="button" className="digital-center-icon-button" aria-label="Vergleich schließen"
            onClick={onClose}><X size={19} aria-hidden="true" /></button></header>
        <table><thead><tr><th>Feld</th><th>Digitalzentrale</th><th>RailKeeper</th></tr></thead>
          <tbody>{rows.map(([label, center, railkeeper]) => <tr key={label}><th>{label}</th>
            <td title={String(center)}>{center}</td><td title={String(railkeeper)}>{railkeeper}</td></tr>)}</tbody></table>
        <footer><button type="button" className="digital-center-button" onClick={onClose}>Schließen</button></footer>
      </section>
    </div>
  );
}
